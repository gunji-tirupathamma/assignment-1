...
app.get("/todos/:todoId/", checkRequestsQueries, async (request, response) => {
  ...
  const getSingleTodoQuery = `SELECT id,
                                        todo,
                                        priority,
                                        status,
                                        category,
  ...
  const dbResponse = await db.get(getSingleTodoQuery);
...
app.get("/agenda/", checkRequestsQueries, async (request, response) => {
  ...
  const getDueDateArray = `SELECT id,
                               todo,
                             priority,
                             status,
                             category,
                             due_date AS dueDate  FROM todo
                       WHERE due_date='${date}';`;
  const todoArray = await db.all(getDueDateArray);
...
app.post("/todos/", checkRequestsQueries, async (request, response) => {
  ...
  const addNewTodoQuery = `INSERT INTO todo(id,todo,priority,status,category,due_date)
                            VALUES(${id},
                                '${todo}',
                                '${priority}',
                                '${status}',
  ...
  const createUser = await db.run(addNewTodoQuery);
